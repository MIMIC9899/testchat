...
import { getFirestore, collection, doc, setDoc, getDoc, getDocs, addDoc, updateDoc, deleteDoc, query, orderBy, limit, where, serverTimestamp, startAfter } from 'https://www.gstatic.com/firebasejs/9.6.1/firebase-firestore.js';
import { getDatabase, ref, push, set, onValue, remove, get, onDisconnect } from 'https://www.gstatic.com/firebasejs/9.6.1/firebase-database.js'; //!!!
...
const firebaseConfig = {
    ...
    databaseURL: "https://chatonline-42b5a-default-rtdb.europe-west1.firebasedatabase.app"
};
...
try {
    ...
    if (user) {
        ...
        const presenceRef = ref(window.rtdb, `presence/${user.uid}`);
        set(presenceRef, { online: true, lastSeen: Date.now() }).catch(error => {
        console.error("Failed to set Realtime Database presence:", error);
        });
        onDisconnect(presenceRef).set({ online: false, lastSeen: Date.now() }).catch(error => {
        console.error("Failed to set onDisconnect:", error);
        });
        // Firestore presence
        console.log("Setting presence online for user:", user.uid);
        setDoc(doc(db, 'presence', user.uid), {
        online: true,
        lastSeen: serverTimestamp()
        }, { merge: true }).catch(error => {
        console.error("Failed to set Firestore presence:", error);
        });
        ...
        if (currentUser) {
        console.log("Setting presence offline for user:", currentUser.uid);
        ...
        // Realtime Database presence
        const presenceRef = ref(window.rtdb, `presence/${currentUser.uid}`);
        set(presenceRef, { online: false, lastSeen: Date.now() }).catch(error => {
            console.error("Failed to set offline Realtime Database presence:", error);
        });
        } else {
        console.warn("No currentUser when attempting to set offline presence");
        }
        showAuthForm();
    }
    });
...
logoutBtn.style.display = 'flex';
const presenceRef = ref(window.rtdb, `presence/${user.uid}`);
console.log("Setting presence for path:", `presence/${user.uid}`, "User:", user.uid);
set(presenceRef, { online: true, lastSeen: Date.now() }).catch(error => {
    console.error("Failed to set Realtime Database presence:", error);
});
console.log("Setting onDisconnect for path:", `presence/${user.uid}`);
onDisconnect(presenceRef).set({ online: false, lastSeen: Date.now() }).catch(error => {
    console.error("Failed to set onDisconnect:", error);
});
console.log("Setting Firestore presence online for user:", user.uid);
setDoc(doc(db, 'presence', user.uid), {
    online: true,
    lastSeen: serverTimestamp()
}, { merge: true }).catch(error => {
...
} else if (!isProcessingLogout) {
isProcessingLogout = true;
navLinks.style.display = 'none';
logoutBtn.style.display = 'none';
// Skip onAuthStateChanged offline updates since logout handles them
showAuthForm();
isProcessingLogout = false;
}
});
...
window.register = async function(email, password, username) {
    ...
    await setDoc(doc(db, 'users', user.uid), {
        username, email, createdAt: serverTimestamp(), messagingPrivacy: 'everyone', bio: '', avatar: '', profileBackground: ''
    });
...
window.logout = async function() {
    ...
    console.log("Attempting logout for user:", userToLogout?.uid);
    if (userToLogout && userToLogout.uid) {
        console.log("Setting Firestore presence offline on logout for user:", userToLogout.uid);
        await setDoc(doc(db, 'presence', userToLogout.uid), {
        online: false,
        lastSeen: serverTimestamp()
        }, { merge: true }).catch(error => {
        console.error("Failed to set offline presence on logout:", error);
        });
        const presenceRef = ref(window.rtdb, `presence/${userToLogout.uid}`);
        console.log("Setting offline Realtime Database presence on logout for path:", `presence/${userToLogout.uid}`);
        await set(presenceRef, { online: false, lastSeen: Date.now() }).catch(error => {
        console.error("Failed to set offline Realtime Database presence on logout:", error);
        });
    }
    ...
    document.getElementById('content').innerHTML = `
        ...
        <div id="posts" class="space-y-4">
            ...
            <div class="post bg-gray-50 dark:bg-gray-700 p-4 rounded fade-in">
                ...
                <p class="text-gray-800 dark:text-gray-200">${escapeHtml(post.text)}</p>
                ...
                ${post.comments.map(comment => `
                    ...
                    <span class="font-semibold">${comment.username}</span>: ${escapeHtml(comment.text)}
...
async function loadProfile(uid) {
    ...
    friendsSnapshot1.docs.forEach(doc => friendIds.add(doc.data().userId2));
    friendsSnapshot2.docs.forEach(doc => friendIds.add(doc.data().userId1));
    ...
    const presenceRef = ref(window.rtdb, `presence/${uid}`);
    const presenceSnapshot = await get(presenceRef).catch(error => {
        console.error("Failed to read Realtime presence for", uid, ":", error);
        return null;
    });
    const presenceDoc = await getDoc(doc(db, 'presence', uid));
    const presence = presenceSnapshot && presenceSnapshot.exists() ? presenceSnapshot.val() : { online: false };
    console.log("Presence for user", uid, ":", presence, "Class:", presence.online ? 'bg-green-500' : 'bg-gray-500');
    ...
    document.getElementById('content').innerHTML = `
        ...
        <div class="flex items-center mb-4">
            <img src="${user.avatar || 'https://picsum.photos/64'}" class="w-16 h-16 rounded-full">
            <div class="ml-4">
            <h2 class="text-2xl font-semibold text-gray-900 dark:text-white">${escapeHtml(user.username)}</h2>
            <span class="w-3 h-3 rounded-full ${presence.online ? 'bg-green-500' : 'bg-gray-500'} inline-block"></span>
            <span class="text-gray-500 dark:text-gray-400 ml-2">${presence.online ? 'В сети' : 'Не в сети'}</span>
            </div>
        </div>
        ...
        ${uid === currentUser.uid ? `
            ...
            <input id="username" type="text" placeholder="Имя пользователя" value="${escapeHtml(user.username) || ''}" maxlength="20" class="w-full p-2 border rounded bg-gray-50 dark:bg-gray-800 dark:border-gray-700 dark:text-white">
            <textarea id="bio" placeholder="О себе" maxlength="200" class="w-full p-2 border rounded mt-2 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 dark:text-white">${escapeHtml(user.bio) || ''}</textarea>
...
window.handleSearch = async function(username, isAutocomplete = false) {
    ...
    resultsContainer.innerHTML = await Promise.all(snapshot.docs.map(async docSnapshot => {
        const user = docSnapshot.data();
        const presenceDoc = await getDoc(doc(db, 'presence', docSnapshot.id));
        const presence = presenceDoc.exists() ? presenceDoc.data() : { online: false };
        console.log("Presence for user", docSnapshot.id, ":", presence);
        return `
        <div class="bg-white dark:bg-gray-900 p-4 rounded-lg shadow mb-2 flex justify-between items-center fade-in">
            ...
            <span class="ml-2 cursor-pointer text-gray-900 dark:text-white" onclick="window.loadProfile('${docSnapshot.id}')">${escapeHtml(user.username)}</span>
            <span class="w-2 h-2 rounded-full ${presence.online ? 'bg-green-500' : 'bg-gray-500'} ml-2"></span>
            </div>
...
window.startChat = async function(targetUserId) {
    ...
    await set(ref(window.rtdb, `chats/${chatRef.id}/participants`), {
        [currentUserId]: true,
        [targetUserId]: true
    });
    openChat(chatRef.id);
...
window.openChat = async function(chatId) {
    ...
    `;
    const messagesRef = ref(window.rtdb, `chats/${chatId}/messages`);
    console.log("Setting up onValue listener for:", `chats/${chatId}/messages`);
    onValue(messagesRef, (snapshot) => {
        console.log("onValue triggered, snapshot:", snapshot.val());
        const messagesDiv = document.querySelector('.messages');
        if (messagesDiv) {
        const messages = snapshot.val() || {};
        console.log("Messages to render:", messages);
        messagesDiv.innerHTML = Object.entries(messages).map(([messageId, m]) => `
            ...
            <div class="inline-block p-2 rounded-lg ${m.sender === currentUser.uid ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-700 dark:text-white'}">
                ${escapeHtml(m.text)}
                ${m.sender === currentUser.uid ? `<button onclick="window.deleteMessage('${chatId}', '${messageId}')" class="text-red-300 hover:text-red-100 ml-2 text-xs">Удалить</button>` : ''}
            </div>
            <small class="block text-gray-500 dark:text-gray-400">${new Date(m.timestamp).toLocaleString()}</small>
            </div>
        `).join('');
...
window.sendMessage = async function(chatId, text) {
    ...
    const messagesRef = ref(window.rtdb, `chats/${chatId}/messages`);
    console.log("Sending message to:", `chats/${chatId}/messages`, "Text:", text);
...
window.deleteMessage = async function(chatId, messageId) {
    try {
    const messageRef = ref(window.rtdb, `chats/${chatId}/messages/${messageId}`);
...
window.acceptFriendRequest = async function(requesterId, friendDocId) {
    ...
    window.loadProfile(requesterId);
    } catch (error) {
...
async function loadFriends() {
    ...
    const friends = await Promise.all([...friendsSnapshot1.docs, ...friendsSnapshot2.docs].map(async friendDoc => {
        ...
        if (friendIds.has(friendId)) return null;
        ...
        id: friendId,
        username: userDoc.exists() ? userDoc.data().username : 'Неизвестный',
        avatar: userDoc.exists() ? userDoc.data().avatar || 'https://picsum.photos/32' : 'https://picsum.photos/32',
    ...
    document.getElementById('content').innerHTML = `
        ...
        <div class="space-y-2">
            ...
            <div class="p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg flex justify-between items-center fade-in">
                ...
                <span class="ml-2 cursor-pointer text-gray-900 dark:text-white" onclick="window.loadProfile('${friend.id}')">${escapeHtml(friend.username)}</span>
        ...
        <div class="space-y-2">
            ...
            <div class="p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg flex justify-between items-center fade-in">
                ...
                <span class="ml-2 cursor-pointer text-gray-900 dark:text-white" onclick="window.loadProfile('${request.id}')">${escapeHtml(request.username)}</span>
                </div>
...
const user = doc.exists() ? doc.data() : { messagingPrivacy: 'everyone'};
